...
if not fold_A.exists():
    raise FileNotFoundError('fold_A does not exist: %s' % fold_A)
if not fold_B.exists():
    raise FileNotFoundError('fold_B does not exist: %s' % fold_B)

splits = [p.name for p in fold_A.iterdir() if p.is_dir()]
# Support both layouts:
# 1) split folders under fold_A/fold_B (e.g. train/val/test)
# 2) flat files directly under fold_A/fold_B
if len(splits) == 0:
    splits = [None]
...
for sp in splits:
    ...
    if not img_fold_A.exists() or not img_fold_B.exists():
        print('skip split=%s, missing folder A/B' % split_name)
        continue
    ...
    print('split = %s, use %d/%d images' % (split_name, num_imgs, len(img_list)))
    img_fold_AB.mkdir(parents=True, exist_ok=True)
    print('split = %s, number of images = %d' % (split_name, num_imgs))
